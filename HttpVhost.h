...
private:
    // 只在系统启动的时候进行被调用
    bool handle_vhost_cfg(const libconfig::Setting& setting, std::shared_ptr<HttpHandler>& handler);
...
public:
    ...
    int find_http_get_handler(std::string vhost,
        ...
        auto iter = vhosts_.find(vhost);
        if (iter != vhosts_.end()) {
            tzhttpd_log_debug("finding get %s:%s in vhosts_", vhost.c_str(), uri.c_str());
            return iter->second->find_http_get_handler(uri, phandler_obj);
        }

        tzhttpd_log_debug("finding get %s:%s in default_vhost_", vhost.c_str(), uri.c_str());
...
private:
    ...
    std::map<std::string, std::shared_ptr<HttpHandler>> vhosts_;
    std::shared_ptr<HttpHandler> default_vhost_;
